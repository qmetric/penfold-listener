...
class EventDeserializerTest extends Specification {
    ...
    @Unroll
    def "should deserialize task payload update event"() {
        given:
        ...
        "task_closed.json"          | new TaskClosed("TaskClosed", "a1", 1L, created, Optional.of("user1"), Optional.of("reason1"))
        "task_closed_minimal.json"  | new TaskClosed("TaskClosed", "a1", 1L, created, Optional.absent(), Optional.absent())
    ...
    @Unroll
    def "should deserialize patch operator value for payload update event using custom specific object mapper"() {
        given:
