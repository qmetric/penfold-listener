...
class EventListenerTest extends Specification {
    ...
    //        eventListener.poll(this.getClass().getResource("/fixtures/events/task_payload_updated.json").text)
    ...
    final EventListener eventListener = new EventListener(eventStoreReader, eventTracker, eventHandler)
    ...
    {
        given:
        eventHandler.handle(_ as EventRecord) >> {throw new RuntimeException()}
        eventTracker.lastTracked() >>> [Optional.absent(), Optional.of(trackingRecord(0))]
